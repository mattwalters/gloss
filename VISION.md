# Gloss — Product Vision & Strategy

_Working name: "Gloss" (a gloss is a scholarly comment written alongside a source text — marginalia that survived centuries because it traveled with the manuscript). Name is provisional pending trademark/collision checks. Alternatives considered: Scholia, Cairn, Rings, Grain, Plumb._

## The thesis in one paragraph

Code review — and eventually the whole software development lifecycle: tickets, projects, cycles — should live _in the git repository itself_, as signed, append-only operations stored as git objects, co-located with the code they describe. The pull request is a platform construct, not a git concept; forges typically keep the code half of every PR in refs while the conversational half lives in a platform database. Gloss closes that gap: an open, documented convention for SDLC data in git, a reference engine that reads and writes it, and good clients on top. Because the transport is just git (push/pull over SSH/HTTPS with credentials users already have), it works with every git host, requires no server, and — critically — puts the entire review and ticket history one `git fetch` away from any coding agent, with no API tokens, no rate limits, and no integration work.
...
**Works with your existing host.** Gloss is not a forge and does not compete for where the repo lives. It rides along inside any repo on any host, as just another git client pushing to its own ref namespace. Adoption cost is a `git config` change, not a migration. If you ever stop using Gloss, your data is already in your repo, in a documented format — there is nothing to export and nothing to lose.

**Relationship to project-management tools.** Tools like Linear demonstrate that teams love fast, local-first, operation-synced workflows — and the industry trend of integrating code review into PM tools shows everyone can see the SDLC graph wants to be one graph. Gloss approaches the same convergence from the other direction: bring the whole graph down into git, where it's versioned, offline-capable, and agent-readable. We deeply respect the polish of these products and are not attempting feature parity; the spec is SDLC-general from day one (issue/project/cycle types defined alongside review types), while the initial focus is review, with issues as the natural companion.

**Relationship to review tooling generally.** A wave of excellent commercial review tools shows how much room there is to improve on the status-quo review experience. Gloss's distinct contribution is the open, host-agnostic data layer underneath: review data as a portable convention rather than a per-tool database, which any tool — including those — could in principle read and write.
...
## Scope architecture: repo objects vs. workspace objects

Reviews belong to code repos. But much of project management is org-scoped — projects, cycles, teams, tickets with no repo at all. Forcing those into a code repo breaks the model. Solution: a **workspace repo** — an ordinary git repo holding org-level objects, while code-bound artifacts stay co-located with code, linked by workspace-global IDs. Precedent: Gerrit's All-Projects/All-Users special repos for site config and cross-cutting state. Spec implication: object IDs and cross-references must be workspace-global from day one, because "ticket in repo A fixed by review in repo B" is the core query of the one-graph vision.

## Risks and open questions

1. **Ref-namespace host compatibility (foundational; spike first).** The host-agnostic thesis assumes major hosts accept pushes to custom ref namespaces (`refs/gloss/*`) and don't GC or hide them destructively. Some hosts restrict certain namespaces (e.g., `refs/pull/*` is read-only on GitHub); behavior varies. One-day spike across GitHub, GitLab, Bitbucket, Gitea/Forgejo, Codeberg, plus bare-SSH. If a major host is restrictive, fallback designs exist (branch-namespace encoding) but must be known early.
...
7. **Naming collision checks** outstanding on "Gloss."
...
Spec + fixtures → engine → CLI → GitHub bridge → TUI, with the ref-namespace spike before everything. Rationale: the moment `gloss sync` can fold a real GitHub PR history into git objects in your own repo, the thesis is demonstrable with real data — and that demo is worth more than beautiful chrome on an empty log.
