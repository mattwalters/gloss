...
func TestOpenCloseMemory(t *testing.T) {
	...
	if v := projection.SchemaVersion(); v != 3 {
	...
	if version != "3" {
		t.Fatalf("expected stored schema_version '3', got %q", version)
	}
}
...
func TestSchemaVersionMismatchRecreates(t *testing.T) {
	...
	// 2. Re-open: should detect mismatch, drop tables, and recreate with schema_version 3
	...
	if version != "3" {
		t.Fatalf("expected recreated schema_version '3', got %q", version)
	}
