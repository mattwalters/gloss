# GLS-3 spike: SSH commit-signing via go-git + system git interop

Prototype and findings for whether gloss can sign op-commits with a user's
...
signature. GLS-22 cannot use it; verification has to be hand-rolled, either
...
verify anything. This is a real design constraint for GLS-22, not just an
...
needs either `ssh-keygen` or an agent conversation). Left as a GLS-22
...
private file isn't there. No special-casing needed on gloss's side beyond
...
once GLS-22 has real fixture-sized histories to try it against.

**8. Namespace: `"git"`.** Signing under the same reserved namespace git
itself uses for commit signing (rather than inventing a `gloss` namespace)
means op-commits stay verifiable with plain `git verify-commit` too, not
just gloss's own code — and hosts that render commit-signature badges may
