...
//     parsed and merged, //go:build lines and GOOS-suffixed names alike, and
//     the package name is taken from the first file. Honouring them with
//     go/build's default context would make the listing depend on the host's
//     GOOS/GOARCH, which is the one thing this tool exists not to do, so the
//     fix is a policy decision rather than a flag.
...
func run(w io.Writer, root string) error {
	...
	fmt.Fprintln(w, "# diff here. A line that changes or disappears is a breaking change, and so")
	fmt.Fprintln(w, "# are two kinds of added line: a method added to an exported interface, and")
	fmt.Fprintln(w, "# a constant inserted into an iota block, which renumbers the ones after it.")
	fmt.Fprintln(w, "# `make api-compat` classifies the delta if you want it named.")
