...
GLS-4 spike tool. Measures whether go-git's local object I/O — the "local
...
behavior at `refs/gloss/*` scale (thousands of refs), and DAG-walk
performance.

This is throwaway spike tooling, not engine code. It writes and deletes its
own synthetic refs/commits under `refs/gloss/bench-writer/cobs/spike/*` and
...
(FFmpeg was used for the GLS-4 measurements: 126k+ commits, ~500MB, a
...
- `--ops` — number of synthetic op-commits to write, chained as one
  ...
  commit-write + ref-create throughput, the shape of one gloss write).
...
`docs/spikes/gls-4-go-git-capability.md`.
