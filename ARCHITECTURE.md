...
Every collaborative object has exactly one **home repo** — the repo whose `refs/writ/*` carry its ops. This is structural, not aesthetic: per-writer refs make pushes conflict-free only because everyone collaborating on an object pushes to and fetches from the same remote; an object whose ops could accumulate "additively" across several repos would need something to replicate them between remotes, and there deliberately is no such thing (no server, no P2P). Reviews and comments home in their code repo. Workspace-scoped objects — issues, projects, cycles, workflow states, labels, settings, the repo registry — home in the **workspace-home**.

Three decisions shape what the workspace-home is:

- **Workspace-home is a role, not a dedicated repo.** Any ordinary git repo can play it. The default is self: `writ init` makes the current repo its own workspace, so a single-repo project or a monorepo org runs a complete workspace with zero extra repos. A team spread across several repos designates one of them (typically the center-of-gravity repo) as home; a dedicated workspace repo is an available option for orgs that want the separation, not the architecture's demand. Precedent for the dedicated form staying mundane: Gerrit's All-Projects, GitHub's org-level `.github` repo. The earlier framing that led with a dedicated repo overstated it; the co-location thesis was always about review data, and workspace objects describe no code — what the workspace-home preserves is every property that matters: still just git, still signed append-only ops, still one `git fetch` from any agent, still no server.
- **The home is a mailbox, not an identity.** Object IDs are random 128-bit strings and ops are self-contained signed commits; nothing about an op encodes which repo it lives in. Re-homing a workspace is `git push new-home 'refs/writ/*:refs/writ/*'` plus a config change — so a team's homing choice (main repo now, dedicated repo later, or back) is an operational preference, never a migration.
- **One workspace = one team.** A workspace is a team: a group of people, a registry of repos, one board. Workflow states, labels, and settings are workspace-global — no `team` object type, no team scope on anything, in v1. Multi-team arrives later, additively, if demand proves out: a `team` object plus an optional `team` field on scoped objects; old clients ignore the unknown field (degraded to showing everything, per forward-compatibility), and pre-existing scopeless objects fold as belonging to a default team. Accepted cost meanwhile: importing a multi-team Linear workspace means one writ workspace per team, and cross-workspace references are not representable. This answers WRIT-104's team-scoping question.
